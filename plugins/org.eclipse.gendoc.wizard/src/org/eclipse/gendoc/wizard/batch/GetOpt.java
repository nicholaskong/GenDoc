 ...
 *  Aïcha Boudjelal (Atos) aicha.boudjelal@atos.net - Copy in Gendoc wizard
 *
 *****************************************************************************/
...
public class GetOpt
{
    ...
    public void printHelp(ArgOpt[] listOpts, PrintStream out)
    {
        ...
        {
            String optional = "";
            if (opt.getArg_type() == ArgOpt.OPTIONAL_ARGUMENT)
            {
                optional = "(optional)";
            }
            out.println(String.format("\t--%s (-%s)\t\t: %s", opt.getLongName(), opt.getShortName(), opt.getDescription() + optional));
        }
    }
